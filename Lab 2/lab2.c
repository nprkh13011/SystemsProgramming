...
size_t my_strlen(char *src) {
    ...
    size_t count = 0;
    // Set the current character to the head of the source string.
    char* current = src;
    // If the source is null, return 0.
    if (!src)
        return 0;
    // Else...
    else {
        // ... while current isn't null, increment the count and move current to the next character.
        while (*current) {
            count++;
            current++;
        }   
 ...
 */
char *my_strcpy(char *dst, char *src) {
    // Initialize a C-string called "current" and set it to the pointer address of "dst".
    char* current = dst;
    ...
    // Else...
        ...
        // ... while the source string is not null, copy the current char from "src" into "dst" and move forward one character.
        while (*src)
            *current++ = *src++;
    }
    // Set the last character to the null-terminated character.
    *current = '\0';
    // Return dst.
    return dst;
}
